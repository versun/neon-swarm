 ...
 * - 解锁三种原因：0=目标死亡 1=飞出准心大圈 2=目标下线/隐身
 * - 命中他人重置累计；打空不断累计
 * - Bot 命中不触发锁定
 */
...
/** A 持续瞄准 +x 方向开火（aim 固定在 aimX/aimY），跑到满足条件或超时 */
function runUntil(
  sim: Sim,
  aId: number,
  ...
  for (let i = 0; i < maxTicks; i++) {
    now += STEP_MS;
    sim.setInput(aId, 0, 0, 0, 1, aim.x, aim.y);
...
describe("准心锁定机制", () => {
  it("对同一敌机连续命中 3 发触发 lock，前 2 发不触发", () => {
    const sim = new Sim();
    ...
    for (let i = 0; i < 600 && lockAtHit < 0; i++) {
      now += STEP_MS;
      sim.setInput(A.id, 0, 0, 0, 1, B.x, B.y);
      const evs = step(sim, now);
      for (const e of evs) {
  ...
  it("锁定后制导弹巡航转向：目标垂直机动仍被持续命中", () => {
    ...
    // 先打出锁定
    runUntil(sim, A.id, { x: 200, y: 0 }, (evs) => evs.some((e) => e[0] === "lock"));
    expect(A.lockTargetId).toBe(B.id);
  ...
  it("目标飞出准心大圈 → unlock reason 1", () => {
    ...
    // aim 固定在 (200,0)，把 B 传送到大圈外（距离 > LOCK_RADIUS）
    place(B, 200, LOCK_RADIUS + 100);
    const evs = runUntil(
      sim,
      A.id,
      { x: 200, y: 0 },
      (e) => e.some((x) => x[0] === "unlock"),
      10,
    );
    const un = evs.find((e) => e[0] === "unlock");
    expect(un).toEqual(["unlock", A.id, B.id, 1]);
  ...
  it("目标死亡 → unlock reason 0（在 kill 事件之前）", () => {
    ...
    runUntil(sim, A.id, { x: 200, y: 0 }, (evs) => evs.some((e) => x_lock(e)));
    expect(A.lockTargetId).toBe(B.id);
    A.hits = 0;

    B.hp = 1; // 下一发即致死
    const evs = runUntil(sim, A.id, { x: 200, y: 0 }, (e) =>
      e.some((x) => x[0] === "kill"),
    );
    ...
    function x_lock(e: GameEvent) {
      return e[0] === "lock";
    }
  });

  it("命中他人重置累计：B×2 → C×1 → B×2 不锁定，再中 B 1 发达 3 连发才锁定", () => {
    const sim = new Sim();
    const A = sim.addUnit("Alpha", false);
    const B = sim.addUnit("Bravo", false);
    ...
    // 阶段 1：中 B 2 发
    runUntil(sim, A.id, { x: 200, y: 0 }, countUntil(B.id, 2));
    expect(A.streakVictimId).toBe(B.id);
    expect(A.streakHits).toBe(2);

    // 阶段 2：B 挪走、C 上场，中 C 1 发 → 累计切换归零
    place(B, 200, 2000);
    place(C, 200, 0);
    runUntil(sim, A.id, { x: 200, y: 0 }, countUntil(C.id, 1));
    expect(A.streakVictimId).toBe(C.id);
    expect(A.streakHits).toBe(1);

    // 阶段 3：C 挪走、B 回场，再中 B 2 发 → 对 B 的累计从 1 重计，仅 2 连，不得锁定
    place(C, 2000, 0);
    place(B, 200, 0);
    runUntil(sim, A.id, { x: 200, y: 0 }, countUntil(B.id, 2));
    expect(A.streakVictimId).toBe(B.id);
  ...
  it("Bot 命中不触发锁定", () => {
    ...
    for (let i = 0; i < 1200 && hits < LOCK_HITS_REQUIRED + 2; i++) {
      now += STEP_MS;
      sim.setInput(D.id, 0, 0, 0, 1, E.x, E.y);
      const evs = step(sim, now);
      for (const e of evs) {
